...
async function setupDB() {
  const dbName = process.env.DB_NAME || 'generic_medicine';
  ...
  await connection.query(`
    CREATE TABLE IF NOT EXISTS medicines (
      ...
      composition TEXT,
      brandPrice DECIMAL(10,2),
      genericPrice DECIMAL(10,2),
      savings INT,
  ...
  await connection.query(`
    CREATE TABLE IF NOT EXISTS market_trends (
      ...
      monthlyVolume JSON,
      manufacturersShare JSON
    )
  `);

  // Load external db.json data if available
  let seedDataToUse = seedData;
  try {
    const dbJsonPath = path.join(__dirname, 'data', 'db.json');
    if (fs.existsSync(dbJsonPath)) {
      const rawData = JSON.parse(fs.readFileSync(dbJsonPath, 'utf8'));
      if (rawData.medicines && rawData.stores && rawData.marketTrends) {
        seedDataToUse = rawData;
        console.log(`Loaded full dataset from db.json: ${rawData.medicines.length} medicines, ${rawData.stores.length} stores.`);
      }
    }
  } catch (err) {
    console.warn('Could not read db.json, falling back to built-in seed data:', err.message);
  }

  // Insert data if empty
  const [medCount] = await connection.query('SELECT COUNT(*) as count FROM medicines');
  if (medCount[0].count === 0) {
    console.log(`Inserting ${seedDataToUse.medicines.length} seed medicines...`);
    for (const med of seedDataToUse.medicines) {
      await connection.query(
        `INSERT INTO medicines (
          id, brandName, genericName, composition, brandPrice, genericPrice, 
          savings, manufacturer, category, details, availability, barcode, 
          dosage, schedule, sideEffects, contraindications, lastSync
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
        [
          med.id,
          med.brandName,
          ...
          med.dosage || null,
          med.schedule || null,
          med.sideEffects ? JSON.stringify(med.sideEffects) : null,
          med.contraindications ? JSON.stringify(med.contraindications) : null
        ]
      );
    }
  }
  ...
  if (storeCount[0].count === 0) {
    console.log(`Inserting ${seedDataToUse.stores.length} seed stores...`);
    for (const store of seedDataToUse.stores) {
      let stockObj = {};
      if (store.stock) {
        stockObj = store.stock;
      } else if (store.availableMedicines) {
        store.availableMedicines.forEach(id => {
          stockObj[id] = 'Available';
        });
      }

      await connection.query(
        'INSERT INTO stores (id, name, type, rating, phone, address, lat, lng, stock) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [
          store.id,
          store.name,
          store.type,
  ...
  const [trendCount] = await connection.query('SELECT COUNT(*) as count FROM market_trends');
  if (trendCount[0].count === 0) {
    console.log('Inserting seed trends...');
    const t = seedDataToUse.marketTrends;
    await connection.query(
      'INSERT INTO market_trends (adoptionRates, categoryAnalysis, regionalPricing, monthlyVolume, manufacturersShare) VALUES (?, ?, ?, ?, ?)',
      [
        JSON.stringify(t.adoptionRates),
        JSON.stringify(t.categoryAnalysis),
        JSON.stringify(t.regionalPricing),
        JSON.stringify(t.monthlyVolume),
        JSON.stringify(t.manufacturersShare)
      ]
    );
  }
